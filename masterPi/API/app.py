...
class Booking(db.Model):
    ...
    #new additions, start here for reference
...
def update_booking_by_bookingId(bookingId):
    ...
    if data.get('mechanicalIssue'):
        get_booking.location= data['mechanicalIssue']         
    if data.get('ticketStatus'):
        get_booking.location= data['ticketStatus']         
    if data.get('location'):
        get_booking.location= data['location']         
    db.session.add(get_booking)
    db.session.commit()
    booking_schema = BookingSchema(only=['bookingId', 'carId', 'userId','bookStatus','startTime','endTime','returnTime','bookingFee','customerNotes','adminNotes','mechanicalIssue','ticketStatus','location'])
